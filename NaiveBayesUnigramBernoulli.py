...
X = cv.fit_transform(data['stemmed']).toarray() #yielads accuracy of 52.8%
X= (X > 0.5).astype(np.int_)  #make all entries binary  yields accuracy of 49.3%
...
model.fit(X_train, y_train)   #Bernoulli
#model2.fit(X_train, y_train)    #Multinomial
